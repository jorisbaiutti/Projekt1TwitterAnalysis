...
public class BubbleChartControllerTest {
    ...
    @Autowired
    BubbleChartController bubbleChartController;
    ...
    public void shouldreturnaListofanalyse() throws Exception {
        List<String> availableendpoints = bubbleChartController.analysen.stream().map(a -> a.getName()).collect(Collectors.toList());
        String responsebody = availableendpoints.toString();


        this.mockMvc.perform(get("/api/bubblechart/list"))
                .andDo(print())
